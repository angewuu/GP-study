...
use "Data\population 1\OX57_gp_clinical_events_14169", clear
destring, replace
gen dataset =2

append using "Angela's coding\dataset\chd_merged_interventions.dta", force

save "Angela's coding\dataset\chd_merged_interventions.dta", replace

use "Data\population 1\OX57_gp_clinical_events_14170", clear
destring, replace
gen dataset = 3
...
use "Data\population 1\OX57_gp_clinical_events_14171", clear
destring, replace
gen dataset = 4
...
use "Data\population 1\OX57_gp_clinical_events_14172", clear
destring, replace
gen dataset = 5
...
use "Data\population 1\OX57_gp_clinical_events_14177", clear
destring, replace
gen dataset = 6
...
use "Data\population 1\OX57_gp_clinical_events_14178", clear
destring, replace
gen dataset = 7
...
use "Data\population 1\OX57_gp_clinical_events_14180", clear
destring, replace
gen dataset = 8

append using "Angela's coding\dataset\chd_merged_interventions.dta", force

use "Data\population 1\OX57_gp_clinical_events_14206", clear
destring, replace
gen dataset = 9
...
use "Data\population 1\gp_drugscripts_target_7645", clear
destring, replace
gen dataset = 10
...
use "Data\population 1\gp_drugscripts_target_7646", clear
destring, replace
gen dataset = 11
...
use "Data\population 1\gp_drugscripts_target_14194", clear
destring, replace
gen dataset = 12
...
merge 1:1 patid using  "Angela's coding\CHD_final_smoking.dta"
...
cap drop yearonefre yeartwofre yearthreefre yearfourfre yearfivefre

cap drop num_effectivedatetime
gen num_effectivedatetime = 0
foreach var of varlist effectivedatetime* {
	replace num_effectivedatetime = num_effectivedatetime + (`var' > chd_date & `var' < chd_date +365) if !missing(`var') & chd_date !=.
	}
...
gen num_effectivedatetime = 0
foreach var of varlist effectivedatetime* {
	replace num_effectivedatetime = num_effectivedatetime + (`var' > chd_date + 365 & `var' < chd_date + 730) if !missing(`var') & chd_date !=.
	}
...
gen num_effectivedatetime = 0
foreach var of varlist effectivedatetime* {
	replace num_effectivedatetime = num_effectivedatetime + (`var' > chd_date + 730 & `var' < chd_date + 1095) if !missing(`var') & chd_date !=.
	}
...
gen num_effectivedatetime = 0
foreach var of varlist effectivedatetime* {
	replace num_effectivedatetime = num_effectivedatetime + (`var' > chd_date + 1095 & `var' < chd_date + 1460) if !missing(`var') & chd_date !=.
	}
...
gen QOFtime =.
replace QOFtime = 1 if chd_date > mdy(03,31,2004)
replace QOFtime = 0 if chd_date <mdy(04,01,2004)
...
cd "P:\OX57"
use "Angela's coding\CHD smoking_2.dta", clear
cap drop firstabsti
...
forval j=0/412 {
	...
	qui replace firstabsti = smoke_date`j' if (`cat_var' == 0  | `cat_var'==1) & smoke_date`j' > chd_date & (`cat_var' !=.) & (firstabsti ==.)
	}

replace firstabstitime = firstabsti - chd_date
...
forval j=0/412 {
	...
	qui replace firstsmokingrecord = smoke_date`j' if (`cat_var' == 2  | `cat_var'==3  | `cat_var'== 4) & smoke_date`j' > chd_date & (`cat_var' !=.) & (firstsmokingrecord==.)
	}

replace firstsmokingtime = firstsmokingrecord - chd_date
...
replace firstabstitime = firstabsti - chd_date if firstabstitime ==.

replace abstinence = 1 if abstinence ==. 

keep patid chd_date firstabsti firstabstitime abstinence withinoneyear withintwoyears withinthreeyears withinfouryears withinfiveyears withinsixyears withinsevenyears 


save "Angela's coding\chd smoking_only.dta", replace

use "Angela's coding\CHD_final_smoking.dta", clear
merge 1:1 patid using "Angela's coding\chd smoking_only.dta"
drop if _merge == 2
drop _merge

save "Angela's coding\CHD_final_smoking.dta", replace
